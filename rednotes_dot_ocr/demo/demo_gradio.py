...
def load_file_for_preview(file_path, session_state):
    ...
    try:
        if file_ext == ".pdf":
            ...
            return (
                None,
                "<div id='page_info_box'>Unsupported file format</div>",
                session_state,
            )
    except Exception as e:
        return (
            None,
            f"<div id='page_info_box'>PDF loading failed: {str(e)}</div>",
            session_state,
        )
...
def turn_page(direction, session_state):
    ...
    if direction == "prev":
        ...
        pdf_cache["current_page"] = min(
            pdf_cache["total_pages"] - 1, pdf_cache["current_page"] + 1
        )

    index = pdf_cache["current_page"]
    current_image = pdf_cache["images"][index]  # Use the original image by default
    page_info = (
        f"<div id='page_info_box'>{index + 1} / {pdf_cache['total_pages']}</div>"
    )
        ...
        if "cells_data" in result and result["cells_data"]:
            try:
                current_json = json.dumps(
                    result["cells_data"], ensure_ascii=False, indent=2
                )
            except:
...
def process_image_inference(
    ...
    if processing_results.get("temp_dir") and os.path.exists(
        processing_results["temp_dir"]
    ):
        try:
    ...
    session_state["processing_results"] = get_initial_session_state()[
        "processing_results"
    ]
    processing_results = session_state["processing_results"]
    ...
    if not input_file_path:
        return (
            None,
            "Please upload image/PDF file or select test image",
            "",
            "",
    ...
    try:
        if file_ext == ".pdf":
            ...
            preview_image, page_info, session_state = load_file_for_preview(
                input_file_path, session_state
            )
            pdf_result = parse_pdf_with_high_level_api(
                dots_parser, input_file_path, prompt_mode
            )
                ...
                if first_result.get("cells_data"):
                    try:
                        current_page_json = json.dumps(
                            first_result["cells_data"], ensure_ascii=False, indent=2
                        )
                    except:
            ...
            if pdf_result["temp_dir"]:
                download_zip_path = os.path.join(
                    pdf_result["temp_dir"],
                    f"layout_results_{pdf_result['session_id']}.zip",
                )
                with zipfile.ZipFile(
                    download_zip_path, "w", zipfile.ZIP_DEFLATED
                ) as zipf:
                        ...
                        for file in files:
                            if not file.endswith(".zip"):
                                zipf.write(
                                    ...
                                    os.path.relpath(
                                        os.path.join(root, file), pdf_result["temp_dir"]
                                    ),
                                )
            ...
            parse_result = parse_image_with_high_level_api(
                dots_parser, image, prompt_mode, fitz_preprocess
            )
            ...
            md_content_raw = (
                parse_result["md_content"] or "No markdown content generated"
            )
            processing_results.update(
                {
                    "original_image": original_image,
            ...
            num_elements = (
                len(parse_result["cells_data"]) if parse_result["cells_data"] else 0
            )
            info_text = f"**Image Information:**\n- Original Size: {original_image.width} x {original_image.height}\n- Model Input Size: {parse_result['input_width']} x {parse_result['input_height']}\n- Server: {current_config['ip']}:{current_config['port_vllm']}\n- Detected {num_elements} layout elements\n- Session ID: {parse_result['session_id']}"

            current_json = (
                json.dumps(parse_result["cells_data"], ensure_ascii=False, indent=2)
                if parse_result["cells_data"]
                else ""
            )
            ...
            if parse_result["temp_dir"]:
                download_zip_path = os.path.join(
                    ...
                    f"layout_results_{parse_result['session_id']}.zip",
                )
                with zipfile.ZipFile(
                    download_zip_path, "w", zipfile.ZIP_DEFLATED
                ) as zipf:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_id="left-panel"):
                ...
                prompt_display = gr.Textbox(
                    ...
                    value=dict_promptmode_to_prompt[
                        list(dict_promptmode_to_prompt.keys())[0]
                    ],
                    lines=4,
                    max_lines=8,
                    interactive=False,
                ...
                with gr.Row():
                    process_btn = gr.Button(
                        "🔍 Parse", variant="primary", scale=2, elem_id="parse_button"
                    )
            ...
            with gr.Column(scale=6, variant="compact"):
                with gr.Row():
                    ...
                    with gr.Column(scale=3):
                        ...
                        info_display = gr.Markdown(
                            "Waiting for processing results...", elem_id="info_box"
                        )
                ...
                with gr.Row():
                    download_btn = gr.DownloadButton(
                        "⬇️ Download Results", visible=False
                    )
