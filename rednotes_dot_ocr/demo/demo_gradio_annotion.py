...
def process_image_inference_with_annotation(
    annotation_data,
    test_image_input,
    prompt_mode,
    server_ip,
    ...
    if image is None:
        return (
            None,
            "Please select a test image or add an image in the annotation component",
            "",
            "",
            gr.update(value=None),
            "",
    ...
    if bbox is None:
        return (
            None,
            "Please select a bounding box by mouse",
            "Please select a bounding box by mouse",
            "",
            "",
            gr.update(value=None),
    ...
    try:
        ...
        parse_result = parse_image_with_bbox(
            dots_parser, image, prompt_mode, bbox, fitz_preprocess
        )
                    ...
                    for root, dirs, files in os.walk(parse_result["temp_dir"]):
                        for file in files:
                            ...
                            arcname = os.path.relpath(
                                file_path, parse_result["temp_dir"]
                            )
    ...
    with gr.Blocks(theme="ocean", css=css, title="dots.ocr - Annotation") as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, variant="compact"):
                ...
                load_btn = gr.Button(
                    "📷 Load Image to Annotation Area", variant="secondary"
                )
